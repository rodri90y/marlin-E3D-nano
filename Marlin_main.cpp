...
void cancel_gohome(void)
{
	...
	if(cancel_gohome_ms<=millis())
	{
		...
		current_position[Z_AXIS] = z_height_stop;
		destination[Z_AXIS] = z_height_stop;
		sync_plan_position();	
	}
	return;
...
void setup() {	
  ...
  set_absolute_position(Z_AXIS,z_height_stop);
...
static void homeaxis(AxisEnum axis) {
  ...
  if (axis == X_AXIS ? HOMEAXIS_DO(X) : axis == Y_AXIS ? HOMEAXIS_DO(Y) : axis == Z_AXIS ? HOMEAXIS_DO(Z) : 0) {
    ...
    destination[axis] = 1.0 * max_length(axis) * axis_home_dir; //(3)设置归零的长度
...
void manage_inactivity(bool ignore_stepper_queue/*=false*/) {
  ...
  if (stepper_inactive_time && ms > previous_cmd_ms + stepper_inactive_time
    ...
    // ---------------------------------------------------------
    static int homeDebounceCount = 3000;   // poor man's debouncing count
